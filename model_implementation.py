...
class code2vec_model(nn.Module):
  ...
  def __init__(self, 
    ...
    configuration = BertConfig(vocab_size=self.values_vocab_size, hidden_size=self.embedding_dim, num_attention_heads=8, intermediate_size=context_size)
  ...
  def forward(self, starts, paths, ends, labels):
    ...
    print (self.bert(attention_mask=mask, inputs_embeds=comb_context_vec).shape)
